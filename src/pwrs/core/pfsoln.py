...
def pfsoln(baseMVA, bus0, gen0, branch0, Ybus, Yf, Yt, V, ref, pv, pq, mpopt=None):
    ...
    bus_type = bus[gen_bus, BUS_TYPE]
    on = np.flatnonzero((gen[:, GEN_STATUS] > 0) & (bus_type != PQ))
    off = np.flatnonzero(gen[:, GEN_STATUS] <= 0)
    gbus = gen[on, GEN_BUS].astype(int)
    gbus_idx = gbus - 1

    Sbus = V[gbus_idx] * np.conj(Ybus[gbus_idx, :] @ V)

    gen[off, QG] = np.zeros(off.size)
    if on.size:
        _, Qd_gbus = total_load_pq(bus[gbus_idx, :], None, "bus", None, mpopt)
        Qd_gbus = np.ravel(Qd_gbus)
        gen[on, QG] = np.imag(Sbus) * baseMVA + Qd_gbus
    ...
    if on.size > 1:
        ...
        Qmin = gen[on, QMIN].copy()
        Qmax = gen[on, QMAX].copy()
        M = np.abs(gen[on, QG])
        ...
        Qg_tot = np.ravel(Cg.T @ gen[on, QG])
        ...
        if ig.size:
            ...
            gen[on[ig], QG] = Qmin[ig] + np.ravel(Cg[ig, :] @ mis)
    ...
    for ref_k in ref:
        ...
        first = on[refgen[0]]
        gen[first, PG] = np.real(Sbus[refgen[0]]) * baseMVA + Pd_refk
        if refgen.size > 1:
            gen[first, PG] = gen[first, PG] - np.sum(gen[on[refgen[1:]], PG])

    out = np.flatnonzero(branch[:, BR_STATUS] == 0)
    br = np.flatnonzero(branch[:, BR_STATUS] != 0)
    if br.size:
        f_idx = branch[br, F_BUS].astype(int) - 1
        t_idx = branch[br, T_BUS].astype(int) - 1
        Sf = V[f_idx] * np.conj(Yf[br, :] @ V) * baseMVA
        St = V[t_idx] * np.conj(Yt[br, :] @ V) * baseMVA
        flow_columns = np.array([PF, QF, PT, QT], dtype=int)
        branch[np.ix_(br, flow_columns)] = np.column_stack(
            [np.real(Sf), np.imag(Sf), np.real(St), np.imag(St)]
        )
    if out.size:
        branch[np.ix_(out, np.array([PF, QF, PT, QT], dtype=int))] = 0.0
